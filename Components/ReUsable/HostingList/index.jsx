...
export default function HostingList({ data }) {
  return (
    <div className="mt-10">
      <div className="mx-4 sm:mx-20 2xl:mx-32">
        <h1 className="mb-4 text-4xl font-extrabold tracking-tight leading-none text-gray-900 md:text-5xl 2xl:text-6xl dark:text-white">
      ...
      {Array.isArray(data?.addHosting) &&
        [...data?.addHosting]
          .sort((a, b) => a?.id - b?.id)
          ?.map((x, i) => (
            <div className="px-3 md:mx-20 2xl:mx-32 my-3" key={i}>
              <Card>
                    ...
                    <div className="w-full lg:w-2/5 mt-3 lg:mt-0">
                      ...
                      >
                        {x?.hostingFeatures?.split("\n").map((a, b) => (
                          <div key={b} className="text-start">
                            <Link
                              isBlock
                              key={i}
                              className="mb-0"
