...
export default function ThemeInfo() {
  ...
  const prestaShopThemeInfo = useSelector((state) => state.prestaShopThemeInfo);
  const loading = useSelector((state) => state.loading);

  useEffect(() => {
    axios
      .get("/api/prestashop/affiliate-db")
      .then((res) => setThemeAffiliate(res.data));
    axios
      .get("/api/prestashop/theme-info-component")
              ...
              >
                <CardBody>
                  <div className="space-y-5 max-w-4xl mx-auto text-center z-50">
                    ...
                    >
                      This PrestaShop site using the theme
                    </h2>
                    <h2 className="text-4xl font-extrabold mx-auto md:text-6xl text-transparent bg-clip-text bg-gradient-to-r from-[#4F46E5] to-[#E114E5] pb-3 text-center">
                      {prestaShopThemeInfo?.data?.themeInfo?.[0]
                        ?.split(/[ -]/)
          ...
          <div className="mt-8 flex justify-center">
            ...
            >
              <Image
                src={
                  prestaShopThemeInfo?.status
                    ? prestaShopThemeInfo?.data?.screenShot?.[0]
