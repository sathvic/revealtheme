  ...
  try {
    ...
    if (response.status === 200) {
      ...
      $("script").each((i, el) => {
        ...
        if (content.includes("M.cfg")) {
          scriptContent = content;
          return false; // Exit the loop once the correct script is found
        }
      });

      // Extract the JSON object from the script content
      const configMatch = scriptContent.match(/M\.cfg\s*=\s*(\{.*?\});/);

      // Parse the JSON object if it was found
      if (configMatch) {
        const config = JSON.parse(configMatch[1]);
        themeName = config.theme;
      } else {
        themeName = "Unable to get Theme Name";
      }
